...
class LikeSongProcessorImpl @Inject constructor(
    ...
    private suspend fun updateLikePlaylist(id: Long, like: Boolean): Boolean {
        val likePlaylist = runCatching {
            MineApi.get().getUserPlaylist(userService.getUserId()).playlists
                .firstOrNull { it.specialType == 5 }
        }.getOrNull() ?: return false
        val result = runCatching {
            MineApi.get().collectSong(
                pid = likePlaylist.id,
                tracks = id.toString(),
