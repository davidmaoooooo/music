...
import me.wcy.music.listen.ListenTogetherRole
...
class MainActivity : BaseMusicActivity() {
    ...
    private fun initContent() {
        ...
        initDrawer()
        ListenTogetherManager.bind(application.playerController())
        parseIntent()
    }
    ...
    private fun initDrawer() {
        ...
        lifecycleScope.launch {
            ListenTogetherManager.state.collectLatest { state ->
                ...
                navigationHeaderBinding.listenTogetherStatusCard.isVisible = active
                navigationHeaderBinding.tvListenTogetherStatus.text = if (active) {
                    val peer = state.peerName.ifEmpty { "等待对方" }
                    val song = state.currentTitle.ifEmpty { "暂无歌曲" }
                    val accepted = if (state.serverAccepted) "服务器已接受" else "等待服务器确认"
                    val latency = if (state.latencyMs >= 0) "延迟 ${state.latencyMs}ms" else "延迟检测中"
                    val role = if (state.role == ListenTogetherRole.Owner) " · 房主" else ""
                    "房间 ${state.roomCode}$role\n$accepted · $latency\n对方：$peer\n正在播放：$song"
                } else {
                    ""
                }
                navigationHeaderBinding.tvListenTogetherExit.isVisible = active
            }
        }
    }

    private fun updateListenTogetherDrawerEntry() {
        viewBinding.navigationView.menu.findItem(R.id.action_listen_together)?.isVisible =
