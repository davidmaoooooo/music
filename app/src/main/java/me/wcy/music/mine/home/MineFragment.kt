...
class MineFragment : BaseMusicFragment() {
    ...
    private fun initPlaylist() {
        ...
        viewBinding.ivLikeHeartMode.setOnClickListener {
            val likePlaylist = viewModel.likePlaylist.value
            if (likePlaylist == null) {
                toast("还没有喜欢的音乐")
            } else {
                startLikeHeartMode(likePlaylist)
            }
        }
    ...
    private fun startLikeHeartMode(likePlaylist: PlaylistData) {
        lifecycleScope.launch {
            showLoading()
            val result = kotlin.runCatching {
                withContext(Dispatchers.IO) {
                    val currentSeedId = playerController.currentSong.value
                        ?.takeIf { it.isLocal().not() }
                        ?.getSongId()
                        ?.takeIf { it > 0 }
                    val seedSong = if (currentSeedId != null) {
                        playerController.currentSong.value
                    } else {
                        DiscoverApi
                            .getFullPlaylistSongList(likePlaylist.id)
                            .songs
                            .filter { it.id > 0 }
                            .randomOrNull()
                            ?.toMediaItem(likePlaylist.id)
                    }
                        ?: throw IllegalStateException("喜欢的音乐为空")
                    seedSong
                }
            }
            dismissLoading()
            result.onSuccess { seedSong ->
                playerController.startHeartMode(seedSong, likePlaylist.id)
