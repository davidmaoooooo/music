...
object LrcCache {
    ...
    fun getTranslatedLrcFilePath(music: MediaItem): String? {
        if (music.isLocal()) {
            return null
        }
        val lrcFile = File(FilePath.lrcDir, "${music.getSongId()}.tlyric")
        if (lrcFile.exists()) {
            return lrcFile.path
        }
        return null
    ...
    suspend fun saveTranslatedLrcFile(music: MediaItem, content: String): File {
        return withContext(Dispatchers.IO) {
