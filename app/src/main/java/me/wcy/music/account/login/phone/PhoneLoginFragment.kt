...
class PhoneLoginFragment : BaseMusicFragment() {
    ...
    private fun initView() {
        ...
        viewBinding.btnCookieLogin.setOnClickListener {
            val cookie = viewBinding.etCookie.text?.toString()?.trim().orEmpty()
            if (cookie.isEmpty()) {
                ToastUtils.show("请先粘贴 Cookie")
                return@setOnClickListener
            }
            lifecycleScope.launch {
                showLoading(false)
                ...
                if (res.isSuccessWithData()) {
                    ...
                    ToastUtils.show(res.msg.orEmpty().ifEmpty { "Cookie 登录失败，请检查 Cookie 是否有效" })
                }
            }
        }
