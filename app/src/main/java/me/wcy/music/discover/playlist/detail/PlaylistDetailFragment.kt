...
class PlaylistDetailFragment : BaseMusicFragment() {
    ...
    private val songItemBinder by lazy { createSongItemBinder() }
    private val adapter by lazy { RAdapter<SongData>() }
    private var visibleSongList: List<SongData> = emptyList()
    ...
    private fun initSongList() {
        viewBinding.llPlayAll.setOnClickListener {
            val playlistId = viewModel.playlistData.value?.id ?: 0L
            val songList = viewModel.songList.value.map { it.toMediaItem(playlistId) }
        ...
        adapter.register(songItemBinder)
        viewBinding.recyclerView.adapter = adapter
        viewBinding.etSongSearch.doAfterTextChanged {
            refreshVisibleSongs()
        }

        lifecycleScope.launch {
            viewModel.songList.collectLatest { songList ->
                refreshVisibleSongs(songList)
            }
        }
        lifecycleScope.launch {
            playerController.currentSong.collectLatest { current ->
                songItemBinder.currentSongId = current?.getSongId() ?: 0L
                adapter.refresh(visibleSongList)
            }
        }
    }

    private fun createSongItemBinder(): PlaylistSongItemBinder {
        return PlaylistSongItemBinder(object : OnItemClickListener2<SongData> {
            override fun onItemClick(item: SongData, position: Int) {
                val fullList = viewModel.songList.value
                val fullIndex = fullList.indexOfFirst { it.id == item.id }
                val playlistId = viewModel.playlistData.value?.id ?: 0L
                val songList = fullList.map { it.toMediaItem(playlistId) }
                if (songList.isNotEmpty() && fullIndex >= 0) {
                    playerController.replaceAll(songList, songList[fullIndex])
                }
            }
    ...
    private fun refreshVisibleSongs(source: List<SongData> = viewModel.songList.value) {
        val keyword = viewBinding.etSongSearch.text?.toString()?.trim().orEmpty()
        visibleSongList = if (keyword.isEmpty()) {
            source
        } else {
            source.filter {
        ...
        adapter.refresh(visibleSongList)
        viewBinding.tvSongCount.text = "(${visibleSongList.size})"
    }
